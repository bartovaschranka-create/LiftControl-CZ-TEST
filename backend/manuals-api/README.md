...
```bash
node scripts/build-manual-index.mjs "C:\Users\bartos\Desktop\manualy_JLG_k_nahrani\450AJ pvc2307.pdf" "450AJ pvc2307.pages.json" --manual "JLG 450AJ Service Manual PVC 2307" --maker JLG --model 450AJ --models "450 AJ,450AJ" --manual-type service --edition "PVC 2307" --serial-range "B300000000 and up" --page-images --image-pages 129-135 --pdftoppm "C:\Users\bartos\.cache\codex-runtimes\codex-primary-runtime\dependencies\native\poppler\Library\bin\pdftoppm.exe"
```

`--page-images` ulozi JPEG obrazova data do `.pages.json`. Pouzivej idealne spolecne s `--image-pages`, aby se do indexu nevkladaly stovky zbytecnych stran manualu. Rozsah ale musi pokryt celou servisni kapitolu, ne jen prvni nalezene strany. Pokud chybi obrazovy podklad pro nekterou prelozenou stranu, backend servisni PDF odmitne a v diagnostice vypise chybejici strany.

Pro PDF, ktere ma vypadat jako originalni strana manualu s ceskym textem na stejnych mistech, vytvor index s layout bloky:

```bash
node scripts/build-manual-index.mjs "C:\Users\bartos\Desktop\manualy_JLG_k_nahrani\450AJ pvc2307.pdf" "450AJ pvc2307.pages.json" --manual "JLG 450AJ Service Manual PVC 2307" --maker JLG --model 450AJ --models "450 AJ,450AJ" --manual-type service --edition "PVC 2307" --serial-range "B300000000 and up" --translated-pages --page-images --image-pages 129-135 --pdftoppm "C:\Users\bartos\.cache\codex-runtimes\codex-primary-runtime\dependencies\native\poppler\Library\bin\pdftoppm.exe"
```

`--translated-pages` ulozi pozice textovych bloku. Pokud OpenAI vrati `translatedPages`, servisni PDF pouzije JPEG originalni strany jako podklad a prekryje zdrojove textove bloky ceskym prekladem. Obrazky, tabulky a anglicke popisky uvnitr obrazku zustanou z originalu.
