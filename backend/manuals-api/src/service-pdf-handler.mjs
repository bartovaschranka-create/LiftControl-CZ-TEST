...
export function createServicePdfHandler(deps = {}) {
  return async function servicePdfHandler(req, res) {
    ...
    let body;
    try {
      body = await readJsonBody(req, config.maxBodyBytes * 4);
    } catch {
    ...
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="${safePdfName(body)}"`);
      res.end(pdf);
    } catch (error) {
...
function safePdfName(body) {
  ...
  return `${ascii(raw).replace(/[^a-z0-9_-]+/gi, '-').replace(/-+/g, '-').slice(0, 80) || 'servisni-postup'}.pdf`;
}
